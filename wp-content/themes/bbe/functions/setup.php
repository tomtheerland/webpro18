...
if ( ! function_exists( 'bbe_pagination' ) ) {
	function bbe_pagination() {
		...
		$paginate_links = paginate_links( array(
			...
			'mid_size' => 5,
			'prev_next' => True,
			'prev_text' => '<i class="glyphicon glyphicon-chevron-left"></i> '. __('Previous Page', 'bbe'),
			'next_text' => '<i class="glyphicon glyphicon-chevron-right"></i> '.__('Next Page', 'bbe'),
			'type' => 'list'
		) );
...
function bbe_check_if_loggedin() {
	global $post;
	if ( isset($post->ID) && is_numeric($post->ID) && get_post_meta($post->ID,'bbe_loggedin_only',TRUE)==1  && !is_user_logged_in() ):
